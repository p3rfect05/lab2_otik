...
print(count_inexact("my mi mo mmuu ", "m"))
...
def get_total_information(file_path: str) -> [int, int] :
    ...
    char_information = {byte: calculate_information(1/256) for byte, _ in char_frequencies.items()}

    # Рассчитываем суммарное количество информации в файле
    total_information = sum(freq * char_information[byte] for byte, freq in char_frequencies.items())
